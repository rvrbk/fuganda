...
function normalizeStatus(payload) {
    ...
    const active =
        toBoolean(source.buyer_has_active_subscription)
        ...
        || String(source.buyer_subscription_status ?? '').toLowerCase() === 'active';
...
export async function getBuyerBillingStatus() {
    // In demo mode, return active status immediately
    const demoEnabled = await isDemoMode();
    if (demoEnabled) {
        return { active: true, raw: { buyer_has_active_subscription: true, buyer_subscription_status: 'active' } };
    }

    try {
        const { data } = await axios.get('/api/buyer/billing/status');
...
export async function subscribeBuyerBilling(payload = {}) {
    // In demo mode, just return active
    const demoEnabled = await isDemoMode();
    if (demoEnabled) {
        return { active: true, raw: { buyer_has_active_subscription: true } };
    }

    const { data } = await axios.post('/api/buyer/billing/subscribe', payload);
    return normalizeStatus(data);
}

export async function initiateBuyerBillingCheckout(payload = {}) {
    ...
    if (demoEnabled) {
        return {
            active: true,
            redirectUrl: null,
            raw: { buyer_has_active_subscription: true }
        };
    }
...
export async function cancelBuyerBilling() {
    // In demo mode, just return inactive
    const demoEnabled = await isDemoMode();
    if (demoEnabled) {
        return { active: false, raw: { buyer_has_active_subscription: false } };
    }

    const { data } = await axios.post('/api/buyer/billing/cancel');
    return normalizeStatus(data);
}

export async function hasActiveBuyerSubscription() {
    ...
    const status = await getBuyerBillingStatus();
